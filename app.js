    ...
    apiai.on('response', (response) => {
        ...
        request({
            ...
            if (error) {
                ...
                console.log('Error Sending: ', response.body.error);
            }
        });
    });
    ...
    _.filter(cleanOrders, function(order) {
        ...
        if (order.Coffee_Type !== undefined && order.Coffee_Size !== undefined) {
            ...
            if (order.number !== undefined) {
                let totalBill1 = matched[0].Price * order.number;
                prepareOrder(totalBill1);
            } else {
                let totalBill1 = matched[0].Price;
                prepareOrder(totalBill1);
            }
        } else if (order.Coffee_Type !== undefined && order.Coffee_Size === undefined) {
            // Find when only type is specified
            let matched = _.where(newProd, {
                Name: order.Coffee_Type,
                Size: 'Grande'
            });
            if (order.number !== undefined) {
                let totalBill2 = matched[0].Price * order.number;
                prepareOrder(totalBill2);
            } else {
                let totalBill2 = matched[0].Price;
                prepareOrder(totalBill2);
            }
            ...
            if (order.number !== undefined) {
                let totalBill3 = matched[0].Price * order.number;
                prepareOrder(totalBill3);
            } else {
                let totalBill3 = matched[0].Price;
                prepareOrder(totalBill3);
            }
            ...
            let matched2 = _.where(newProd, {
                Name: order.Types.Coffee_Type,
                Size: order.Types.Coffee_Size
            });
            if (order.Types.number !== undefined) {
                let totalBill4 = (matched[0].Price + matched2[0].Price) * order.Types.number;
                prepareOrder(totalBill4);
            } else {
                let totalBill4 = matched[0].Price + matched2[0].Price;
                prepareOrder(totalBill4);
            }
            ...
            if (order.Quantity.number !== undefined) {
                let totalBill4 = (matched[0].Price + matched2[0].Price) * order.Quantity.number;
                prepareOrder(totalBill4);
            } else {
                let totalBill4 = matched[0].Price + matched2[0].Price;
                prepareOrder(totalBill4);
